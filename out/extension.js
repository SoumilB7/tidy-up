    ...
    const disposable = vscode.commands.registerCommand('tidy-up.removeComments', async () => {
        ...
        try {
            ...
            if (originalText.length > 1000000) { // 1MB limit
                const proceed = await vscode.window.showWarningMessage('This file is very large. Processing may take a while. Continue?', 'Continue', 'Cancel');
                if (proceed !== 'Continue') {
                    return;
                }
            }
...
function removePythonComments(line) {
...
function removeJavaScriptComments(line) {
...
function removeCSSComments(line) {
...
function removeShellComments(line) {
    // Remove # comments, but preserve $ comments
...
function removeYamlComments(line) {
...
function removeJSONComments(line) {
    // Remove // and /* */ comments (non-standard JSON)
    return removeJavaScriptComments(line);
}
function removeCommentsAuto(line) {
...
function isEntirelyComment(line, languageId) {
    const trimmedLine = line.trim();
